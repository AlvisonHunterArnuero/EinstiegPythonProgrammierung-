...
class PawPatrol:
    ...
    def bark(self):
        print("-"*80)
        print(f"For Catchy phrases, {self.name} usually says: ")
    ...
    def description(self):
        print(
            f"{self.name} is a {self.breed} pup that serves as {self.profession} in the Paw Patrols.")
        print("-"*80)


def main():
    catchphrase_list = ["No job is too big, no pup is too small!", "PAW Patrol, to the Lookout!",
                        "PAW Patrol is on a roll!", "PAW Patrol, to the PAW Patroller!"]

    # Declare a list of lists containing the information for the main characters of the series, shall we?
    pups_lst = [['Marshall', 'Dalmatian', 'FireFighter'], ['Rubble', 'Bulldog', 'Constructor'], ['Chase', 'German Shepherd', 'Police'], [
        'Rocky', 'Mongrel', 'Handyman'], ['Zuma', 'Labrador Retriever', 'Aquatic Rescuer'], ['Skye', 'Cockapoo', 'Aviator']]
